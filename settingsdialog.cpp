...
static const char blankString[] = QT_TRANSLATE_NOOP("Настройки", "N/A");
                               ...
                               m_intValidator(new QIntValidator(0, 4000000, this))
{
    ...
    connect(this, SIGNAL(sig_confShow()), this, SLOT(slot_confShow()));

    m_currentSettings.baudRate = serSpeed;
...
//-------------------------------------------------------------------------------------------
//   Диструктор класса окна
//
SettingsDialog::~SettingsDialog()
{

    serSpeed = m_currentSettings.baudRate;
    delete m_ui;
}
//-------------------------------------------------------------------------------------------
//   Метод-слот для обновления и отбражения формы формы объекта
//
void SettingsDialog::slot_confShow()
{
    m_ui->retranslateUi(this);//refresh ui after translate

    if (curPort != -1) showPortInfo(curPort);//update form
...
//-------------------------------------------------------------------------------------------
//   Метод для отображения информации о заводских данных последовательного порта
//
void SettingsDialog::showPortInfo(int idx)
{
    ...
    m_ui->descriptionLabel->setText(tr("Описание: %1").arg(list.count() > 1 ? list.at(1) : tr(blankString)));
    m_ui->manufacturerLabel->setText(tr("Изготовитель: %1").arg(list.count() > 2 ? list.at(2) : tr(blankString)));
    m_ui->serialNumberLabel->setText(tr("Серийный номер: %1").arg(list.count() > 3 ? list.at(3) : tr(blankString)));
    m_ui->locationLabel->setText(tr("Размещение: %1").arg(list.count() > 4 ? list.at(4) : tr(blankString)));
...
//-------------------------------------------------------------------------------------------
//   Мето-слот для обновления параметров последовательного порта
//
...
//-------------------------------------------------------------------------------------------
//   Метод-слот для проверки на валидность значения устанавливаемой скорости передачи данных
//
...
//-------------------------------------------------------------------------------------------
//   Метод-слот для проверки на валидность имени порта
//
...
//-------------------------------------------------------------------------------------------
//   Метод для отображения возможных атрибутов последовательного порта
//
...
/*
    ...
    //m_ui->baudRateBox->addItem(QStringLiteral("230400"), 256000);
...
//-------------------------------------------------------------------------------------------
//   Метод для отображения информации о последовательном порте
//
...
//-------------------------------------------------------------------------------------------
//   Метод-слот для обработки сигнала "обновить параметры порта"
//
